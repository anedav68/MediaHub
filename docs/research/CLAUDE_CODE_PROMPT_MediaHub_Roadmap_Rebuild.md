...
- Survey `src/mediahub/` to learn the **real** module layout. Note especially the existing `recognition/` vs `recognition_swim/` split, and modules like `context_engine/`, `content_engine/`, `workflow/`, `publishing/`, `brand/`, `theming/`, `graphic_renderer/`, `remotion/`, `voice/` — the new concepts must map onto these where they already exist, not spawn a parallel structure.
- **Produce a short written plan before editing.** Verify against the actual code; do not assume.

---

## 2. The strategic reframe (the "why")
- **The hub is a content-strategy brain**, not a results parser. It assesses what a given sports team should post, drafts it, and (where permitted) publishes it largely autonomously.
...
- **Human-approval-gated by default, with a per-content-type toggle** that can flip any single post type to fully autonomous.
...
Rewrite it so that:
- It is organised around the report's **Phase 0–5** (0: de-risk licensing/cost · 1: strategy brain + post-type taxonomy + sport profiles · 2: autonomy toggles + orchestration backbone · 3: broaden ingestion beyond swimming · 4: direct-to-platform publishing to drop Buffer · 5: local-AI substitution). These **supersede** the old Parity → Distinction → Leadership spine; keep one line noting that lineage.
- It reflects **actual current state**, not the report's aspirations. As a *starting hypothesis to verify against the code*: shipped = brand-DNA/guidelines ingestion, voice imitation, the AI operating profile, the adaptive theming engine (§1.6), swim recognition, `graphic_renderer` (Playwright→PNG), Remotion reels, `edge-tts` voiceover, the Buffer publishing path, content packs. Not yet shipped = the content-strategy brain as hub, multi-sport beyond swimming, per-content-type autonomy toggles, direct-to-platform publishing, the local-AI substitution layer. Confirm each against the code and mark items with the existing badges (✅ done · 🔵 in progress · ⚠️ stuck · ❌ not started).
...
- `docs/AUTONOMY_MODEL.md` — the `AutonomyLevel` states (`draft_only`, `approval_required`, `fully_autonomous`), what the per-type toggle controls (whether a human-approval step runs, the confidence threshold to auto-publish, which guardrails fire), the human-in-the-loop checkpoints, and the guardrails for autonomous posting (data-provenance/trust-ledger check, brand-safety/profanity, rate limiting, global kill switch, immutable audit trail).
...
- **No new mandatory paid dependency.** Anything paid (Remotion Company License, Replicate, hosted LLM keys, Buffer, X API) must be **optional**, behind a flag/env var, with a documented **free default**.
...
- [ ] All five new docs exist, match house style, and cross-link each other and the research report.
...
- [ ] `docs/DEPENDENCY_LICENSING.md` reproduces the ADOPT/CAUTION/AUDIT/AVOID register and flags Remotion, edge-tts, Buffer, Replicate, and hosted LLM keys with free substitutes.
